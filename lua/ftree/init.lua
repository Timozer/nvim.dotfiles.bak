...
local colors = require "ftree.colors"
...
---@deprecated
M.on_keypress = require("ftree.actions").on_keypress
...
function M.open_replacing_current_buffer()
  ...
  require("ftree.actions.find-file").fn(bufname)
end
...
local function find_existing_windows()
  return vim.tbl_filter(function(win)
    ...
    return api.nvim_buf_get_name(buf):match "FTree" ~= nil
...
local function update_base_dir_with_filepath(filepath, bufnr)
  ...
  if not vim.startswith(filepath, core.get_explorer().cwd) then
    require('ftree.actions.dir').change_dir(vim.fn.fnamemodify(filepath, ":p:h"))
  end
end
...
function M.open_on_directory()
  ...
  require('ftree.actions.dir').force_dirchange(bufname, true)
end
...
function M.on_enter(netrw_disabled)
  ...
  -- Session that left a FTree Buffer opened, reopen with it
...
local function setup_vim_commands()
  vim.cmd [[
    command! -nargs=? -complete=dir FTreeOpen lua require'ftree'.open("<args>")
    command! FTreeClose lua require'ftree.view'.close()
    command! FTreeToggle lua require'ftree'.toggle(false)
    command! FTreeFocus lua require'ftree'.focus()
    command! FTreeRefresh lua require'ftree.actions.reloaders'.reload_explorer()
    command! FTreeClipboard lua require'ftree.actions.copy-paste'.print_clipboard()
    command! FTreeFindFile lua require'ftree'.find_file(true)
    command! FTreeFindFileToggle lua require'ftree'.toggle(true)
    command! -nargs=1 FTreeResize lua require'ftree'.resize("<args>")
    command! FTreeCollapse lua require'ftree.actions.collapse-all'.fn()
    command! FTreeCollapseKeepBuffers lua require'ftree.actions.collapse-all'.fn(true)
  ]]
end

function M.change_dir(name)
  require('ftree.actions.dir').change_dir(name)
...
function M.setup(conf)
  ...
  require("ftree.log").setup(opts)

  log.line("config", "default config + user")
  log.raw("config", "%s\n", vim.inspect(opts))

  require("ftree.actions").setup(opts)
  require("ftree.colors").setup()
  require("ftree.diagnostics").setup(opts)
  require("ftree.explorer").setup(opts)
  require("ftree.git").setup(opts)
  require("ftree.view").setup(opts)
  require("ftree.lib").setup(opts)
  require("ftree.renderer").setup(opts)
