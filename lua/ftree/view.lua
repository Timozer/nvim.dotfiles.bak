...
M.View = {
  ...
  winopts = {
    ...
    wrap = false,
    winhl = table.concat({
      "EndOfBuffer:FTreeEndOfBuffer",
      "Normal:FTreeNormal",
      "CursorLine:FTreeCursorLine",
      "VertSplit:FTreeVertSplit",
      "StatusLine:FTreeStatusLine",
      "StatusLineNC:FTreeStatuslineNC",
      "SignColumn:FTreeSignColumn",
      "NormalNC:FTreeNormalNC",
    }, ","),
  },
}
...
local function wipe_rogue_buffer()
  for _, bufnr in ipairs(a.nvim_list_bufs()) do
    if not matches_bufnr(bufnr) and a.nvim_buf_get_name(bufnr):match "FTree" ~= nil then
...
local function create_buffer(bufnr)
  ...
  a.nvim_buf_set_name(M.get_bufnr(), "FTree_" .. tab)

  for option, value in pairs(BUFFER_OPTIONS) do
    vim.bo[M.get_bufnr()][option] = value
  end

  require("ftree.actions").apply_mappings(M.get_bufnr())
end
...
--- Restores the state of a FTree window if it was initialized before.
function M.restore_tab_state()
  local tabpage = a.nvim_get_current_tabpage()
  M.set_cursor(M.View.tabpages[tabpage].cursor)
end

--- Returns the window number for ftree within the tabpage specified
...
--- Checks if ftree is displaying the help ui within the tabpage specified
---@param tabpage number: (optional) the number of the chosen tabpage. Defaults to current tabpage.
---@return number
...
function M._prevent_buffer_override()
  ...
  vim.schedule(function()
    ...
    if not bufname:match "FTree" then
      M.View.tabpages = {}
    end
    ...
    require("ftree.actions.open-file").fn("edit", bufname)
  end)
end
