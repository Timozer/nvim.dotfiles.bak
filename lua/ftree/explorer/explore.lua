...
local builders = require "ftree.explorer.node-builders"
local common = require "ftree.explorer.common"
local sorters = require "ftree.explorer.sorters"
...
local function populate_children(handle, cwd, node, status)
    ...
    while true do
        ...
        if not filters.should_ignore(abs) and not filters.should_ignore_git(abs, status.files) and not nodes_by_path[abs]
        then
...
local function walk(cwd, callback)
end
