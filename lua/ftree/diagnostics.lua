...
local sign_names = {
  { "FTreeSignError", "FTreeLspDiagnosticsError" },
  { "FTreeSignWarning", "FTreeLspDiagnosticsWarning" },
  { "FTreeSignInformation", "FTreeLspDiagnosticsInformation" },
  { "FTreeSignHint", "FTreeLspDiagnosticsHint" },
}
...
local function add_sign(linenr, severity)
  ...
  table.insert(signs, vim.fn.sign_place(1, "FTreeDiagnosticSigns", sign_name, buf, { lnum = linenr + 1 }))
end
...
local links = {
  FTreeLspDiagnosticsError = has_06 and "DiagnosticError" or "LspDiagnosticsDefaultError",
  FTreeLspDiagnosticsWarning = has_06 and "DiagnosticWarn" or "LspDiagnosticsDefaultWarning",
  FTreeLspDiagnosticsInformation = has_06 and "DiagnosticInfo" or "LspDiagnosticsDefaultInformation",
  FTreeLspDiagnosticsHint = has_06 and "DiagnosticHint" or "LspDiagnosticsDefaultHint",
}
...
function M.setup(opts)
  ...
  if M.enable then
    if has_06 then
      vim.cmd "au DiagnosticChanged * lua require'ftree.diagnostics'.update()"
    else
      vim.cmd "au User LspDiagnosticsChanged lua require'ftree.diagnostics'.update()"
    end
  end
end
